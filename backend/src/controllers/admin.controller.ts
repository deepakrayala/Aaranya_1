...
export async function getAdminDashboard(_request: Request, response: Response): Promise<void> {
  const [metrics, revenue, recentOrders, topProducts] = await Promise.all([
    ...
    db.query(
      `SELECT orders.id, orders.order_number, orders.total_amount_paise, orders.status, orders.shipping_city,
        orders.created_at, users.name AS customer_name, users.email AS customer_email,
    ...
    db.query(
      `SELECT order_items.product_id, order_items.product_name AS name, order_items.sku,
  ...
  response.json({
    ...
    revenue_definition: "Statuses 2, 3, 4, and 5: PAID, PROCESSING, SHIPPED, DELIVERED.",
  });
}

export async function listAdminCustomers(request: Request, response: Response): Promise<void> {
  const search = typeof request.query.search === "string" ? request.query.search.trim().slice(0, 100) : "";
  const tier = typeof request.query.tier === "string" ? request.query.tier : "All";
  const values: unknown[] = [];
  const filters = ["users.role = 2"];

  if (search) {
    values.push(`%${search}%`);
    filters.push(`(users.name ILIKE $${values.length + 1} OR users.email ILIKE $${values.length + 1})`);
  }
  ...
  const result = await db.query(
    `WITH order_stats AS (
       SELECT user_id,
        ...
        COALESCE(SUM(total_amount_paise) FILTER (WHERE status = ANY($1::smallint[])), 0)::bigint AS total_spent_paise,
     ...
     address_stats AS (
       SELECT user_id, (ARRAY_AGG(city ORDER BY created_at DESC))[1] AS latest_address_city
       FROM addresses
       GROUP BY user_id
     )
     SELECT users.id, users.name, users.email, users.role, users.created_at,
      ...
      COALESCE(order_stats.latest_order_city, address_stats.latest_address_city, '') AS city,
      ${computedTier} AS tier
     FROM users
     LEFT JOIN order_stats ON order_stats.user_id = users.id
     LEFT JOIN address_stats ON address_stats.user_id = users.id
...
export async function getAdminCustomer(request: Request<{ id: string }>, response: Response): Promise<void> {
  ...
  const result = await db.query(
    `WITH order_stats AS (
     ...
     LEFT JOIN order_stats ON order_stats.user_id = users.id
     LEFT JOIN address_stats ON address_stats.user_id = users.id
     WHERE users.id = $2 AND users.role = 2
     GROUP BY users.id, order_stats.order_count, order_stats.total_spent_paise, order_stats.last_order_at, order_stats.latest_order_city, address_stats.latest_address_city`,
...
export async function getAdminAnalytics(request: Request, response: Response): Promise<void> {
  const rangeDays = clampRange(request.query.range);

  const [revenueSeries, statusDistribution, topProducts, categoryPerformance] = await Promise.all([
    db.query(
      `WITH days AS (
      ...
      SELECT TO_CHAR(days.day, 'Mon DD') AS d,
        COALESCE(COUNT(orders.id), 0)::int AS orders,
        COALESCE(SUM(orders.total_amount_paise) FILTER (WHERE orders.status = ANY($2::smallint[])), 0)::bigint AS revenue_paise
      FROM days
    ...
    db.query(
      ...
      SELECT statuses.status, statuses.label, COALESCE(COUNT(orders.id), 0)::int AS count
      FROM statuses
      LEFT JOIN orders ON orders.status = statuses.status
    ...
    db.query(
      `SELECT categories.name AS category_name,
        COALESCE(SUM(order_items.quantity) FILTER (WHERE orders.id IS NOT NULL), 0)::int AS quantity_sold,
        COALESCE(SUM(order_items.price_paise * order_items.quantity) FILTER (WHERE orders.id IS NOT NULL), 0)::bigint AS revenue_paise
      FROM categories
      INNER JOIN products ON products.category_id = categories.id
      LEFT JOIN order_items ON order_items.product_id = products.id
      LEFT JOIN orders ON orders.id = order_items.order_id AND orders.status = ANY($1::smallint[])
        AND orders.created_at >= NOW() - ($2::int * INTERVAL '1 day')
