    ...
    <div className="rounded-lg border border-cream/8 bg-[#161310] p-5">
      ...
      <div
        className={`mt-2 inline-flex items-center gap-1 text-xs ${
          positive ? "text-emerald-400/90" : "text-amber-300/90"
        }`}
      >
        {positive ? (
...
function AdminDashboard() {
  ...
  if (dashboard.isPending || analytics.isPending) {
    return (
      <div className="px-6 py-8 text-cream/55">
        Loading dashboard...
      </div>
    );
  }

  if (dashboard.isError || analytics.isError) {
    return (
      <div className="px-6 py-8 text-rose-300">
  ...
  const aovPaise =
    metrics.orders > 0
      ? Math.round(
          paiseToNumber(metrics.revenue_paise) / metrics.orders,
        )
      : 0;
  ...
  return (
    <div className="space-y-8 px-6 py-8 md:px-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-cream/55">
            PostgreSQL operations snapshot - IST
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          label="Revenue"
          value={formatINRPaise(metrics.revenue_paise)}
          note="paid through delivered"
          icon={ShoppingBag}
        />
        ...
        <Kpi
          ...
          note={`${metrics.pending_orders} pending`}
          icon={Package}
        />

        <Kpi
          label="Avg order value"
          value={formatINRPaise(aovPaise)}
          note="successful revenue / orders"
          icon={ArrowUpRight}
        />

        <Kpi
          label="Customers"
          value={metrics.customers.toLocaleString("en-IN")}
          note={`${metrics.products} products`}
          icon={Users}
          positive={metrics.low_stock_products === 0}
        />
      </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueSeries}>
                ...
                <YAxis
                  ...
                  axisLine={false}
                  tickFormatter={(v) =>
                    `INR ${Math.round(Number(v) / 1000)}k`
                  }
                />
                ...
                <Tooltip
                  ...
                  formatter={(v: number, name) =>
                    name === "revenue"
                      ? formatINRPaise(Math.round(v * 100))
                      : v
                  }
                />
        ...
        <div className="rounded-lg border border-cream/8 bg-[#161310] p-5">
          ...
          <ul className="mt-2 space-y-1.5 text-xs">
            {fulfillmentData.map((item, i) => (
              <li
                key={item.label}
                className="flex items-center justify-between text-cream/65"
              >
                <span className="flex items-center gap-2">
                  <span
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-lg border border-cream/8 bg-[#161310] p-5 lg:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <Link
              to="/admin/products"
              className="text-xs uppercase tracking-[0.2em] text-terra hover:text-terra/80"
            >
              All products {"->"}
            </Link>
          </div>
          ...
          <div className="mt-4 h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={dashboard.data.top_products}
                layout="vertical"
                margin={{ left: 20 }}
              >
                <CartesianGrid
        ...
        <div className="rounded-lg border border-cream/8 bg-[#161310] p-5">
          ...
          <ul className="mt-4 space-y-3">
            {dashboard.data.recent_orders.map((o) => (
              <li
                key={o.id}
                className="flex items-center justify-between border-b border-cream/8 pb-3 text-sm last:border-0"
              >
                <div>
                  ...
                  <div className="text-xs text-cream/45">
                    Order #{o.order_number} - {o.shipping_city}
                  </div>
                </div>

                <div className="text-right">
                  <div className="text-cream">
                    {formatINRPaise(o.total_amount_paise)}
                  </div>

                  <StatusPill
                    status={
                      orderStatuses[
                        o.status as keyof typeof orderStatuses
                      ]
                    }
                  />
                </div>
            ...
            {dashboard.data.recent_orders.length === 0 && (
              <li className="text-sm text-cream/45">
                No orders yet.
              </li>
            )}
          </ul>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          {
            title: `${metrics.low_stock_products} low-stock products`,
            body: "Threshold: 10 units or fewer.",
            tone: metrics.low_stock_products ? "amber" : "emerald",
          },
          {
            title: `${metrics.pending_orders} pending orders`,
            body: "Orders waiting for payment confirmation.",
            tone: metrics.pending_orders ? "amber" : "emerald",
          },
          {
            title: `${metrics.delivered_orders} delivered orders`,
            body: "Delivered status from PostgreSQL orders.",
            tone: "emerald",
          },
        ].map((a) => (
          <div
            key={a.title}
            className="rounded-lg border border-cream/8 bg-[#161310] p-5"
          >
            <div
...
export function StatusPill({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    Delivered:
      "bg-cream/10 text-cream/70 border-cream/15",
    Cancelled:
